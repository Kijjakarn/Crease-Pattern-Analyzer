...
struct Line: Hashable, CustomStringConvertible {
    ...
    func yFrom(x x: Double) -> Double {
        return (distance - x*unitNormal.x)/unitNormal.y
    }

    func xFrom(y y: Double) -> Double {
        return (distance - y*unitNormal.y)/unitNormal.x
    }

    func reflectionOf(point: PointVector) -> PointVector {
        return point + unitNormal*2*(distance - point.dot(unitNormal))
    }

    func projectionOf(point: PointVector) -> PointVector {
...
func angle(first: Line, _ second: Line) -> Double {
...
func intersection(first: Line, _ second: Line) -> PointVector? {
