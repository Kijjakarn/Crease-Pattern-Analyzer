...
struct Line: CustomStringConvertible {
    ...
    var description: String {
        if unitNormal.y == 0 {
            ...
            if intercept == 0 || abs(intercept) < εF { return "x = 0" }
        ...
        if slope == 0 {
            if intercept == 0 || abs(intercept) < εF { return "y = 0" }
            if intercept <  0 { return "y = -\(-intercept)" }
            return "y = \(intercept)"
        }
        if intercept == 0 || abs(intercept) < εF { return "y = \(slope)x" }
    ...
    func isParallel(toLine line: Line) -> Bool {
        return abs(unitNormal.dot(line.unitNormal.rotatedBy90())) < ε
    }

    func contains(point: PointVector) -> Bool {
        return distance(toPoint: point) < ε
    }
