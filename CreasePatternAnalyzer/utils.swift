...
// Combine two instructions if the latter is made by
// makeInstructions(point:pointLabels:), except for the last instruction
func coalesceInstructions() {
    /* // Coalesce instructions for points that can be made using the same crease
    for i = 0..<main.instructions.count {
        if main.diagrams[i].fold == nil {
            var accumulatedPoints = [(PointVector, String)]()
            for j = (i + 1)..<main.instructions.count {
                if main.diagrams[j].fold == nil
                && main.diagrams[j].lines.contains(main.diagrams[i].lines[0].0)
                && main.diagrams[j].lines.contains(main.diagrams[i].lines[1].0)
                {
                    accumulatedPoints.append(main.)
                    main.diagrams.remove(at: j)
                }
            }
        }
    } */
...
func makeInstructions(for reference: Reference) {
    ...
    while !stack.isEmpty {
        ...
        case .A1(let point1, let point2):
            ...
            line.label = "\(lineLabels.popLast()!)"
            instruction = "(A1) Fold through \(point1) and \(point2), creating "
                        + "\(line)."
        ...
        case .A3(let line1, let line2):
            ...
            // Else only one of line1's reflections is valid and only one of line2's
            // reflections is valid
        ...
        case .A6(let point1, let line1, let point2, let line2):
            ...
            line.label = "\(lineLabels.popLast()!)"
            instruction = "(A6) Simultaneously fold \(point1) onto \(line1) "
                        + "and \(point2) onto \(line2), creating \(line)."
        ...
        case .A7(let point1, let line1, let line2):
            ...
            line.label = "\(lineLabels.popLast()!)"
            instruction = "(A7) Fold \(point1) onto \(line1) perpendicular to "
                        + "\(line2), creating \(line)"
