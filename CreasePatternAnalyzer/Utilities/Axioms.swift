...
func axiom1(_ p1: PointVector, _ p2: PointVector) -> Line? {
    let fold = Line(p1, p2)
...
func axiom3(_ line1: Line, _ line2: Line) -> [Line] {
    ...
    var folds = [Line]()
    if main.paper.contains(line: fold1) { folds.append(fold1) }
    if main.paper.contains(line: fold2) { folds.append(fold2) }
    return folds
}

// Axiom 4: Given a point p and a line, we can make a fold perpendicular to the
//          line passing through the point p
func axiom4(point: PointVector, line: Line) -> Line? {
    if main.paper.encloses(point: line.projection(ofPoint: point)) {
        let fold = Line(point: point, unitNormal: line.unitNormal.rotatedBy90())
        return main.paper.contains(line: fold) ? fold : nil
    }
    return nil
}
...
func axiom5(bring p1: PointVector, to line: Line, through p2: PointVector)
    ...
    if main.paper.encloses(point: point2) {
        let p12 = (p1 + point2)/2
        if p2 != p12 {
...
func solveCubic(_ a: Double , _ b: Double, _ c: Double, _ d: Double) -> [Double]
{
...
func axiom7(bring p: PointVector, to line1: Line, along line2: Line) -> Line? {
    ...
    if (main.paper.encloses(point: fold.reflection(ofPoint: p))
    && main.paper.contains(line: fold)) {
        return fold
    }
    return nil
}
